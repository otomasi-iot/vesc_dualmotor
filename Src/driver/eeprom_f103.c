...
static FLASH_Status EE_ErasePageIfNotEmpty(uint32_t PageAddress);

/**
 * @brief  Erase a Flash page if it's not already empty
...
static FLASH_Status EE_ErasePageIfNotEmpty(uint32_t PageAddress)
{
    ...
    /* Check if page is already erased */
    uint32_t *addr = (uint32_t *)PageAddress;
    uint32_t page_size_words = FLASH_PAGE_SIZE / 4;
    bool is_erased = true;
    ...
    if (is_erased) {
        return HAL_OK;  /* Page already erased */
    }
...
uint16_t EE_Init(void)
{
    ...
    case ERASED:
        ...
        {
            /* Erase Page0 */
            FlashStatus = EE_ErasePageIfNotEmpty(PAGE0_BASE_ADDRESS);
        ...
        else /* First EEPROM access or invalid state -> format EEPROM */
        {
    ...
    case RECEIVE_DATA:
        if (PageStatus1 == VALID_PAGE) /* Page0 receive, Page1 valid */
        {
            ...
            /* Erase Page1 */
            FlashStatus = EE_ErasePageIfNotEmpty(PAGE1_BASE_ADDRESS);
            if (FlashStatus != FLASH_COMPLETE)
            {
                return FlashStatus;
            }
...
{
    FLASH_Status FlashStatus = FLASH_COMPLETE;
    uint32_t NewPageAddress = 0x080107FF, OldPageAddress = 0x08010000;
    ...
    FlashStatus = EE_ErasePageIfNotEmpty(OldPageAddress);
    /* If erase operation was failed, a Flash error code is returned */
    if (FlashStatus != FLASH_COMPLETE)
    {
        return FlashStatus;
