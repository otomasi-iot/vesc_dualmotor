...
static void *adc_thread(void *arg) {
	...
	while(1) {
		// Sleep for a time according to the specified rate (convert to milliseconds)
		...
		if (ramp_time > 0.01) {
			const float ramp_step = (float)(osKernelGetTickCount() - last_time) / (ramp_time * 1000.0);
